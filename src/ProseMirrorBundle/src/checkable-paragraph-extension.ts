...
/**
 * Convenience functions for the CheckableParagraph class.
 */
export class CheckableParagraphHelper {
  /*
  ...
  */
  public static setCheckStateForAllToTodo(editor: Editor): void {
    this.setCheckStateForAll(editor, '');
  }
  ...
  * @param {Editor}  editor - A TipTap editor instance.
  */
  public static setCheckStateForAllToDone(editor: Editor): void {
    this.setCheckStateForAll(editor, 'done');
  }
  ...
  * @param {Editor}  editor - A TipTap editor instance.
  */
  public static setCheckStateForAllToDisabled(editor: Editor): void {
    this.setCheckStateForAll(editor, 'disabled');
  }
  ...
  */
  ...
  /**
   * Moves the currently selected paragraph (checklist item) upwards or downwards.
   * @param {Editor} editor - A TipTap editor instance.
   * @param {boolean} upwards - Moves the paragraph upwards if true, otherwise downwards.
   * @param {boolean} singleStep - True if the paragraph should move 1 step upwards/downwards,
   * false if it should move to the top/bottom.
   */
  public static moveChecklist(editor: Editor, upwards: boolean, singleStep: boolean): void {
    if (upwards) {
      this.moveChecklistUp(editor, singleStep);
    }
    else {
      this.moveChecklistDown(editor, singleStep);
    }
  }

  private static moveChecklistUp(editor: Editor, singleStep: boolean): void {
    editor.view.focus();
    const paragraphPos = editor.view.state.selection.$anchor;
    const paragraphInfo = findParentNodeClosestToPos(paragraphPos, node => this.isParagraph(editor, node));
    if (!paragraphInfo)
      return; // Selection is not on paragraph

    const previousParagraphInfo = this.searchPreviousParagraph(
      editor, paragraphPos, singleStep, node => this.isParagraph(editor, node));

    if (previousParagraphInfo) {
      this.moveParagraphUp(editor, paragraphInfo, previousParagraphInfo, true);
    }
  }

  private static moveParagraphUp(editor: Editor, paragraphInfo: any, previousParagraphInfo: any, selectMovedNode: boolean): void {
  ...
  private static moveChecklistDown(editor: Editor, singleStep: boolean): void {
    editor.view.focus();
    const paragraphPos = editor.view.state.selection.$anchor;
    const paragraphInfo = findParentNodeClosestToPos(paragraphPos, node => this.isParagraph(editor, node));
    if (!paragraphInfo)
    ...
    const followingParagraphInfo = this.searchFollowingParagraph(
      editor, paragraphPos, singleStep, node => this.isParagraph(editor, node));
  ...
  * @param {Editor} editor - A TipTap editor instance.
  */
  public static sortPendingToTop(editor: Editor): void {
    const paragraphNodePositions = this.collectAllParagraphs(editor);
    ...
    paragraphNodePositions.forEach(nodePosition => {
      ...
      const paragraphInfo = findParentNodeClosestToPos(paragraphPos, node => this.isParagraph(editor, node));
      const nodeState = this.numericCheckState(node.attrs.htmlElementClass);

      const previousParagraphInfo = this.searchPreviousParagraph(
        editor, paragraphPos, false, previousNode => this.shouldMovePendingToTop(editor, nodeState, previousNode));

      if (previousParagraphInfo) {
        this.moveParagraphUp(editor, paragraphInfo, previousParagraphInfo, false);
      }
    });
  }

  private static collectAllParagraphs(editor: Editor): NodeWithPos[] {
    const result: NodeWithPos[] = [];
    editor.view.state.doc.descendants((node, pos, parent) => {
      if (this.isParagraph(editor, node)) {
  ...
  private static shouldMovePendingToTop(editor: Editor, nodeState: number, previousNode: Node): boolean {
    if (!this.isParagraph(editor, previousNode))
      return false;
    const previousNodeState = this.numericCheckState(previousNode.attrs.htmlElementClass);
  ...
  * @param {Editor}  editor - A TipTap editor instance.
  */
  public static sortAlphabetical(editor: Editor): void {
    const paragraphNodePositions = this.collectAllParagraphs(editor);
    ...
    paragraphNodePositions.forEach(nodePosition => {
      ...
      const previousParagraphInfo = this.searchPreviousParagraph(
        editor, paragraphPos, false, previousNode => this.shouldMoveAlphabetical(editor, node, previousNode));
  ...
  private static shouldMoveAlphabetical(editor: Editor, node: Node, previousNode: Node): boolean {
    if (!this.isParagraph(editor, previousNode))
      return false;
  ...
  */
  private static searchPreviousParagraph(editor: Editor, pos: ResolvedPos, singleStep: boolean, filter: Predicate) {
    let result = null;
    let resultPos: ResolvedPos | null = null;

    // Check if there is a previous matching node.
    const paragraphInfo = findParentNodeClosestToPos(pos, node => this.isParagraph(editor, node));
  ...
  *   See the result of findParentNodeClosestToPos() for comparison.
  */
  private static searchFollowingParagraph(editor: Editor, pos: ResolvedPos, singleStep: boolean, filter: Predicate) {
    let result = null;
  ...
  private static isOfCheckState(node: Node, checkState: string) {
    return node.attrs.htmlElementClass == checkState;
  }

  private static numericCheckState(state: string): number {
  ...
  public static isParagraph(editor: Editor, node: Node) {
    return this.isNodeOfType(node, editor.view.state.schema.nodes.paragraph);
  }

  private static isNodeOfType(node: Node, type: NodeType): boolean {
...
function rotateNodeState(editor: Editor, pos: number) {
  ...
  const nodeInfo = findParentNodeClosestToPos(resolvedPos, node => CheckableParagraphHelper.isParagraph(editor, node));
